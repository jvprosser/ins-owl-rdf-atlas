"""Named write: insert one litigation_task row (no free SQL)."""
...
_TASK_TYPES = frozenset({"COMPLETE_FILE", "ESCALATE_DISCOVERY", "DRAFT_HOLD"})
...
def insert_litigation_task_sql(database: str, row: dict[str, Any]) -> str:
    db = validate_ident(database, "database")
    case_id = row.get("litigation_case_id")
    case_sql = "NULL" if case_id in (None, "") else str(int(case_id))
    due = row.get("due_date")
    due_sql = "NULL" if due in (None, "") else f"CAST({sql_quote(str(due)[:10])} AS DATE)"
    return (
        f"INSERT INTO {db}.litigation_task ("
        "litigation_task_id, litigation_case_id, claim_id, task_type_code, "
        "task_status_code, due_date, run_id, created_at"
        ") VALUES ("
        f"{int(row['litigation_task_id'])}, {case_sql}, {int(row['claim_id'])}, "
...
def create_litigation_task(
    ...
    if task_type not in _TASK_TYPES:
        return json.dumps(
            {
                "error": "event_json.task_type_code must be COMPLETE_FILE, "
                "ESCALATE_DISCOVERY, or DRAFT_HOLD",
            }
        )

    row = {
        "litigation_task_id": _task_id(rid, str(claim_id), task_type),
        "litigation_case_id": body.get("litigation_case_id"),
    ...
    sql = insert_litigation_task_sql(db, row)
    ...
    return json.dumps(
        {
            ...
            "run_id": rid,
            "database": db,
            "table": "litigation_task",
            "litigation_task_id": row["litigation_task_id"],
