...
Studio coworker matching uses the **Role** field, not the Name. Prefer Intake Agent Role exactly `Intake Agent`.
...
routing rules, or Observation results. Intake Agent Role is exactly Intake Agent.
...
- Each NEW user message that names a claim_id for status, intake, or
  ...
  not reuse Intake Agent Observations or case JSON from earlier messages in
