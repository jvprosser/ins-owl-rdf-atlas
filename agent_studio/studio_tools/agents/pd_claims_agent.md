...
run_named_write. RequestPoliceReport letters go through save_claim_letter
(no mail send). You never invent SQL or tool JSON. Never Delegate. Never
invent Observation results. Do not run structured claim intake. If a tool
returns error or 401, Final Answer with that JSON and stop. YAML probes
already chose next_step. You do not re-decide the lane.
```
...
`save_claim_letter` (RequestPoliceReport only).
...
on DetermineFault or PdClaimsReview.
