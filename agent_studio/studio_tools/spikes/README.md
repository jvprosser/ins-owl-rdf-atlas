# Agent Studio spikes (ADR 0001)

| Spike | Folder | Question | Result |
|---|---|---|---|
| Closed | `s1_mcp_from_tool/` | Can `tool.py` call registered MCP? | **FAIL** — no in-process bridge; agent calls MCP then Python tools |
| Closed | `s2_git_requirements/` | Does `requirements.txt` accept git+https? | **PASS** — pin `ins-claims-agent` from git |

Upload **only** each spike’s `tool.py` + `requirements.txt` to Studio. Artifacts land in `SESSION_DIRECTORY` (`/workspace`).

Confirm paste via header `CONTENT_ID` / `REPO_REF` / `UPDATED` (and runtime `tool_fingerprint`).
